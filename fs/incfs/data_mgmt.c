...
static int validate_hash_tree(struct backing_file_context *bf, struct file *f,
				   ...
				   struct mem_range tmp)
{
	...
	struct mount_info *mi = NULL;
	struct backing_file_context *bf = NULL;
	struct data_file_block block = {};
	struct data_file *df = get_incfs_data_file(f);
	...
	if (block.db_comp_alg == COMPRESSION_NONE) {
		...
		if (result == bytes_to_read) {
			...
			if (result < 0) {
				const char *name =
					bf->bc_file->f_path.dentry->d_name.name;
...
{
	struct backing_file_context *bf = df->df_backing_file_context;
